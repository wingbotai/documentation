...
|`PAGE_{page.id}_RESET_ON_CONTEXT_CHANGE_PROPS`||string|When contents of any provided variable changes, the chat will start over. (`null|undefined` to *any* is not a change)|`resetOnContextChangeProps`||
...
|`PAGE_{page.id}_OAUTH_STORE_USER_ID_AT_CONTEXT_PROP`||string|OAuth - stores user ID after successful authorization at a thread context variable|`oauth.storeUserIdAtContextProp`||
|`PAGE_{page.id}_OAUTH_STORE_AUTH_RESULT_AT_CONTEXT_PROP`||string|OAuth - stores contents of ID token at a thread context variable|`oauth.storeAuthResultAtContextProp`||
